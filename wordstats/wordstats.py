...
class WordStats(commands.Cog):
    ...
    async def wordstats(
        ...
        for value in result:
            ...
            msg += f'{value[1]}{" " * (maxwidth-currentwidth)}{value[0]}\n'
        if amount == 1:
            mc = "**most common** word"
            is_are = "is"
        else:
    ...
    async def wordstats_global(
        self, ctx, amount_or_word: Optional[Union[int, str]] = 30
    ):
        """
        ...
        async with ctx.typing():
            if word:
                ...
                if not result:
                    return await ctx.send(f"The word **{word}** has not been said yet.")
                rank, count = result
                ordinal = lambda n: "%d%s" % (
                    n,
                    "tsnrhtdd"[(n // 10 % 10 != 1) * (n % 10 < 4) * n % 10 :: 4],
    ...
    async def topchatters(
        ...
        for value in result:
            ...
            msg += f'{value[1]}{" " * (maxwidth-currentwidth)}{name}\n'
        if word:
    ...
    async def topchatters_global(
        self, ctx, word: Optional[str] = None, amount: int = 10
    ):
        """
    ...
    async def topratio(
        ...
        async with ctx.typing():
            ...
            for user_id in worddict:
                if (
                    word in worddict[user_id]
                    and sum(worddict[user_id].values()) >= min_total
                ):
                    sumdict[user_id] = worddict[user_id][word] / sum(
                        worddict[user_id].values()
                    )
    ...
    async def topratio_global(
        self, ctx, word: str, amount: int = 10, min_total: int = 0
    ):
        """
        ...
        if value is None:
            ...
            if value:
                ...
                if ctx.channel.id in v:
                    await ctx.send(
                        "Stats are already not being recorded in this channel."
                    )
                else:
    ...
    async def user(self, ctx, value: bool = None):
        ...
        if value is None:
            ...
            if v:
                await ctx.send(
                    f"Stats are being recorded for you. Use `{ctx.prefix}wordstatsset user no` to disable."
                )
            else:
    ...
    async def forgetme(self, ctx):
        ...
        This cannot be undone.
        """
        await self.red_delete_data_for_user(
            requester="user_strict", user_id=ctx.author.id
        )
        await ctx.send(
    ...
    @commands.guild_only()
    @commands.command(name="topworduser")
    async def topworduser(self, ctx, word: str):
        """
        Shows the user who has used a specific word the most in this server.
        """
        word = word.lower()
        result = self.cursor.execute(
            "SELECT user_id, SUM(quantity) as total FROM member_words "
        ...
        if result:
            ...
            await ctx.send(
                f'User {name} has used the word "{word}" the most in this server, a total of {count} times.'
            )
        else:
            await ctx.send(f'No records found for the word "{word}" in this server.')
    ...
    async def red_delete_data_for_user(self, *, requester, user_id):
        ...
        query = "DELETE FROM member_words " "WHERE user_id = ?;"
